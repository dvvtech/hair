(function(){
  ...
  function renderHistory(){
    ...
    if(!list.length){ const empty=document.createElement('div'); empty.className='card p-6 text-center text-neutral-600 dark:text-neutral-400'; empty.textContent='История пуста'; root.appendChild(empty); return; }
    list.forEach(it=>{
      ...
      const time=document.createElement('time'); time.className='text-sm text-neutral-600 dark:text-neutral-400'; time.textContent=new Date(it.ts).toLocaleString();
